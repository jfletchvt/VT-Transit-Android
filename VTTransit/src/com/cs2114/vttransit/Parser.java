...
/**
 * A parser object, which can be created and then used to parse through the
 * VTransit API (the XML's created upon server requests) via the helper class
 * TBXML.java
 *
...
public class Parser
{
        ...
        IOException
    {
        ...
        String xmlURL =
            "http://www.bt4u.org/webservices/BT4U_WebService.asmx/GetNextDepartures?routeShortName="
        ...
        if (root != null)
        {
            TBXMLElement nextDepartures =
                tbxml.childElement("NextDepartures", root);
            while (nextDepartures != null)
            {
                TBXMLElement time =
                    tbxml.childElement("AdjustedDepartureTime", nextDepartures);
                arrivalTimes.add(tbxml.textForElement(time));
                nextDepartures =
                    tbxml.nextSibling("NextDepartures", nextDepartures);
            }
        }
        return arrivalTimes;
    }
        ...
        IOException
    {
        ...
        String xmlURL =
            "http://www.bt4u.org/webservices/BT4U_WebService.asmx/GetScheduledStopNames?routeShortName="
                + route.getShortName();
        String xmlString = fileToString(xmlURL);
        TBXML tbxml = new TBXML(xmlString);
        TBXMLElement root = tbxml.rootXMLElement();
        if (root != null)
        {
            TBXMLElement scheduledStop =
                tbxml.childElement("ScheduledStops", root);
            while (scheduledStop != null)
            {
                TBXMLElement stopName =
                    tbxml.childElement("StopName", scheduledStop);
                TBXMLElement stopCode =
                    tbxml.childElement("StopCode", scheduledStop);
                stopsOnRoute.add(new Stop(tbxml.textForElement(stopName), tbxml
                    .textForElement(stopCode)));
                scheduledStop =
                    tbxml.nextSibling("ScheduledStops", scheduledStop);
            }
        }
        return stopsOnRoute;
    }
